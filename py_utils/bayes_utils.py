...
class BayesSampler:
        ...
        for i in range(len(total_index) - period * 2 + 1):
            for flag in flags:
                for group in ['test', 'ctrl']:
                    sliced_pre = agg_df.xs(flag, drop_level=False).xs(
                        slice(total_index[i], total_index[i + period - 1]), level=level, drop_level=False).xs(group,
                                                                                                              level=level + 1,
                                                                                                              drop_level=False).groupby(
                        seg).agg({'pv': 'sum', 'year': 'first', 'week': 'first'})
                    sliced_post = agg_df.xs(flag, drop_level=False).xs(
                        slice(total_index[i + period], total_index[i + 2 * period - 1]), level=level,
                        drop_level=False).xs(group, level=level + 1, drop_level=False).groupby(seg).agg(
                        {'pv': 'sum', 'year': 'last', 'week': 'last'})
    ...
    def _preprocess_no_seg(self, df, group_col: list, index_col: str, period: int, file: str):
        ...
        for i in range(len(total_index) - period * 2 + 1):
            for group in ['test', 'ctrl']:
                sliced_pre = agg_df.loc[slice(total_index[i], total_index[i + period - 1])].xs(group, level=1,
                                                                                               drop_level=False).reset_index(
                    drop=False)
                sliced_post = agg_df.loc[slice(total_index[i + period], total_index[i + 2 * period - 1])].xs(group,
                                                                                                             level=1,
                                                                                                             drop_level=False).reset_index(
                    drop=False)
                sliced_post = sliced_post.rename(columns={'pv': 'post pv sum'})
                ...
                res = pd.concat([res, new_sliced])
        res = res.drop(
            columns=['year_x', 'week_x', 'year_y', 'week_y', 'test_ctrl_x', 'test_ctrl_y', 'Date_x', 'Date_y'])
    ...
    def calc_diff_log_no_zip(self, hist, test, seg):
        hist_by_time = hist.groupby(['test_ctrl'] + seg + ['start_year', 'start_week', 'end_year', 'end_week']).agg(
            {'log_lift': 'mean'})  # , 'std_log_lift': 'first', 'count_zip': 'first'})
        ...
        if len(seg) > 0:
            hist_by_time_diff['diff_log_lift_std'] = hist_by_time_diff.reset_index(drop=False).groupby(seg)[
                'diff_log_lift'].agg('std')
        else:
            hist_by_time_diff['diff_log_lift_std'] = np.std(hist_by_time_diff['diff_log_lift'])

        test_by_time = test.groupby(['test_ctrl'] + seg + ['start_year', 'start_week', 'end_year', 'end_week']).agg(
            {'log_lift': 'mean'})
        test_by_time_test = test_by_time.loc['test']
        test_by_time_ctrl = test_by_time.loc['ctrl']
        test_by_time_diff = test_by_time_test.merge(test_by_time_ctrl, right_index=True, left_index=True,
                                                    suffixes=('_test', '_ctrl'))
    ...
    def calc_diff_log(self, hist, test, seg):
        ...
        hist_by_time_diff = hist_by_time_test.merge(hist_by_time_ctrl, right_index=True, left_index=True,
                                                    suffixes=('_test', '_ctrl'))
        hist_by_time_diff['diff_log_lift'] = hist_by_time_diff['log_lift_test'] - hist_by_time_diff['log_lift_ctrl']
        hist_by_time_diff['diff_log_lift_std'] = (((hist_by_time_diff['count_zip_test'] * hist_by_time_diff[
            'std_log_lift_test'] ** 2) + (hist_by_time_diff['count_zip_ctrl'] * hist_by_time_diff[
            'std_log_lift_ctrl'] ** 2)) / (hist_by_time_diff['count_zip_test'] + hist_by_time_diff[
            'count_zip_ctrl'])) ** 0.5
        ...
        test_by_time_diff = test_by_time_test.merge(test_by_time_ctrl, right_index=True, left_index=True,
                                                    suffixes=('_test', '_ctrl'))
        test_by_time_diff['diff_log_lift'] = test_by_time_diff['log_lift_test'] - test_by_time_diff['log_lift_ctrl']
        test_by_time_diff['diff_log_lift_std'] = (((test_by_time_diff['count_zip_test'] * test_by_time_diff[
            'std_log_lift_test'] ** 2) + (test_by_time_diff['count_zip_ctrl'] * test_by_time_diff[
            'std_log_lift_ctrl'] ** 2)) / (test_by_time_diff['count_zip_test'] + test_by_time_diff[
            'count_zip_ctrl'])) ** 0.5
    ...
    def calc_diff_no_zip(self, hist, test, seg):
        hist_by_time = hist.groupby(['test_ctrl'] + seg + ['start_year', 'start_week', 'end_year', 'end_week']).agg(
            {'lift': 'mean'})  # , 'std_log_lift': 'first', 'count_zip': 'first'})
        hist_by_time_test = hist_by_time.loc['test']
        hist_by_time_ctrl = hist_by_time.loc['ctrl']
        hist_by_time_diff = hist_by_time_test.merge(hist_by_time_ctrl, right_index=True, left_index=True,
                                                    suffixes=('_test', '_ctrl'))
    ...
    def result_calc_no_zip(self, hist_by_time_diff, test_by_time_diff):
        flags = self.flags
        res = {'seg': [], 'hist_mean': [], 'hist_sd': [],
               'test_mean': []}  # , 'test_sd':[], 't_score':[], 'p_value':[]}
        for flag in flags:
    ...
    def result_calc(self, hist_by_time_diff, test_by_time_diff):
        ...
        for flag in flags:
            ...
            city_hml_hist_mean, city_hml_hist_sd = city_hml_hist_posterior.loc['mu', 'mean'], \
                                                   city_hml_hist_posterior.loc['sd', 'mean']
            city_hml_test_mean, city_hml_test_sd = test_by_time_diff.loc[flag]['diff_log_lift'].values[0], \
                                                   test_by_time_diff.loc[flag]['diff_log_lift_std'].values[0]
            t_score, p_value = ttest_ind_from_stats(city_hml_test_mean, city_hml_test_sd,
                                                    city_hml_test['dof'].values[0], city_hml_hist_mean,
                                                    city_hml_hist_sd, city_hml_hist['dof'].values[0])
            res['seg'].append(flag)
            res['hist_mean'].append(city_hml_hist_mean)
            res['hist_sd'].append(city_hml_hist_sd)
            res['test_mean'].append(city_hml_test_mean)
